...
//TO count number of characters in each word
System.out.println("TO count number of characters in each word :");
		names.stream().collect(Collectors.toMap(x-> x, x-> x.length())).forEach((k,v) -> System.out.println(k + " : " +v));;
//To count words using map
System.out.println("To count words using map :");
names.stream().map(x->x.toUpperCase()).collect(Collectors.toMap(x-> x, x-> 1, (oldValue, newValue) -> oldValue + newValue)).forEach((k,v) -> System.out.println(k + " : " +v));;


		// Ea]xample to count occurence of chars
		//String sentence = "My Name is Manish , I love Java";
		String sentence2 = "My Name is Manish , My lanuage is java ,I Love Java";

		char[] charArray = sentence2.replaceAll("\\W+", "").toCharArray();
		...
		List<String> asList = Arrays.asList(sentence2.split(" "));
		

		Map<Integer, Long> collect11 = asList.stream().map(x -> x.toUpperCase())
				.collect(Collectors.groupingBy(String::length, Collectors.counting()));
		System.out.println("occurence of words :" + collect11);
	}
